...
public class MainActivity extends AppCompatActivity {
    ...
    /*
    부가데이터를 전달했던 액티비티로부터 결과값을 돌려받기 위해 오버라이딩 한다.
    해당 메서드는 setResult()가 호출되면 자동으로 콜백된다.
    즉, 임의로 호출하지 않는다.
    매개변수
    ...
    전달했던 데이터를 돌려받기 위한 메서드
     */
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        super.onActivityResult(requestCode, resultCode, data);

        //내가 보낸 요청 코드를 받은 액티비티인지 확인 후 텍스트 뷰에 출력
        //REQUEST_CODE는 전역변수로 0으로 초기화 된 상태이다. (상단에서 확인가능)
