...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //결과값 전송 버튼에 리스너 부착
        //결과값을 다시 메인액티비티로 전달한다.
        findViewById(R.id.btn_finish_for_result).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                /*
                아이디, 패스워드에 대한 일치여부를 판단한 후
                setResult()를 통해 결과값을 메인액티비티로 전송한다.
                이때 메인 액티비티에 오버라이딩 된 onActivityResult()가
                자동으로 콜백된다.

                별도로 호출하지 않아도 자동으로 콜백됨.
                 */
                if(!"kosmo".equals(user)){
                    ...
                    intent.putExtra("SUCCESS", user+"님 하이롱");
